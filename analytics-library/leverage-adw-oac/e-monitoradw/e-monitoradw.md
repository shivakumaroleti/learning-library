...
![ADW Monitor](../e-monitoradw/images/ociadwhmonitor.png)
...
["Lab 5: Gaining Insights with Visual Data Dialog"]("../../d-gaininsigt/d-gaininsigt.md")
...
1. ### Lunch Database Service Console
    ...
    ![Autonomous Data Warehouse](../e-monitoradw/images/ocisignin.png)
    ...
    ![Autonomous Data Warehouse](../e-monitoradw/images/ociadwhsmall.png)

    1.3  Select your Compartment and Click on your ADW instance  
    ![Select ADW](../e-monitoradw/images/ociadwhselect.png)

    1.4 In your ADW **Database Details** page, click the **Service Console** button.  
    ![Service Console](../e-monitoradw/images/ociadwhserviceconsole.png)
...
2. ### Examine the Console Overview and Activity Page
    ...
    ![Service Console Overview](../e-monitoradw/images/ociadwhserviceconsoleoverview.png)

    2.2 To access detailed information about the service's performance, click the **Activity** tab in the Service Console.  
    ![Service Console Activity](../e-monitoradw/images/ociadwhserviceconsoleactivitysmall.png)

    2.3 Click the **Monitored SQL** tab to see information about current and past monitored SQL statements.  
    ![Monitored SQL](../e-monitoradw/images/ociadwhmonitoredsql.png)
    ...
    ![Inspect](../e-monitoradw/images/projectdatasetinspectsmall.png)

    2.7 Go to **General** tab and notice the **Data Access** is set _Automatic Caching_.  
    ![Data Access](../e-monitoradw/images/projectdatasetinspect2small.png)
    ...
    ![Data Access Live](../e-monitoradw/images/projectdatasetinspect3small.png)
    ...
    ![New Queries](../e-monitoradw/images/ociadwhmonitoredsql3.png)
...
1. ### OAC Canvas
   ...
   ![Auto Apply Data](../e-monitoradw/images/autoapplyoff.png)  
   > notice the button turned greyed out ![Auto Apply Data](../e-monitoradw/images/autoapply2small.png)
   ...
   ![Auto Apply Data](../e-monitoradw/images/dayofayear.png)

    1.3 You get the message _**"Click the Auto-Apply Data below to refresh data"**_ with a clear indication where to go.  
   ![Auto Apply Data](../e-monitoradw/images/autoapplyoff2.png)

   1.4 Refresh the data by clicking on **Auto Apply data** and notice that the report takes a while to run.![Auto Apply Data](../e-monitoradw/images/autoapply3small.png)
...
2. ### ADW Service Console
   ...
   ![Service Console Activity](../e-monitoradw/images/ociadwhmonitoredsql4.png)

   2.3 Lets **Scale-Up** the ADW to use the maximum number of OCPUs that you can spare (16 OCPUs is a good number, but not a mandatory one). It takes about 90 seconds for the scale up process to complete.  
   ...
   ![Monitored SQL](../e-monitoradw/images/ociadwh-scaleupsmall.png)

   2.4 Fill in the form with the following information:  
   ...
   Click **Update**
   ![Monitored SQL](../e-monitoradw/images/ociadwh-scaleup16ocpusmall.png)

   2.5 Back in OAC page, lets **refresh** the data and see how fast the new queries get generated in a scaled-up ADW instance.  
   Open the browser page with your OAC canvas and click **Refresh Data** button from top right.  
   ![Refresh Data](../e-monitoradw/images/oac-refreshdatasmall.png)

   2.6 Back in the ADW **Service Console > Activity > Monitored SQL**, note the duration that  the query ran with the updated compute resources.  
   The new query generated has taken just a few millie seconds to complete with 16 OCPUs being utilized.  
   ![Service Console Activity](../e-monitoradw/images/ociadwh-scaleup2.png)
...
1. ### OAC Save Project
    ...
    ![Remove Analytics](../e-monitoradw/images/remove-analyticssmall.png)

    1.2 Lets save our project for further analysis.  
    Click on **Save** icon from upper top right of the screen.  
    ![Save Project](../e-monitoradw/images/save-project0.png)

    1.2 Save Project as Training01 (or any name we want to).  
    Type **Name**: **Training01** an Click **Save** button.  
    ![Save Project](../e-monitoradw/images/save-projectsmall.png)

2. ### OAC Query Log

    Lets now check the query logs generated inside of OAC.

    1.1 Click **Go Back** left arrow from upper left of the screen to go to the Analytics Home Page. ![Go Back](../e-monitoradw/images/gobacksmall.png)

    1.2 Click **Navigator** and then Select **Console**.  
    ![Console](../e-monitoradw/images/navigator-consolesmall.png)

    1.3 Click **Sessions and Query Cache** under _Configuration and Administration_ section.  
    ![Sessions and Query Cache](../e-monitoradw/images/sessionandquery.png)

    1.4 The logs screen shows all the query logs and it can be sorted in ascending/descending order.  
    Locate the **Cursor Cache** section, and review the query information recorded there.  
    ![Cursor Cache](../e-monitoradw/images/managesessions.png)

    1.5 Sort the query log in a descending order.  
     **Sort By**> Select **Order Time Descending**.  
    ![Order Time Descending](../e-monitoradw/images/managesessionsdesc.png)

    1.6 Pick-up the most recent one and Click **View Log**.  
    ![View Log](../e-monitoradw/images/managesessionsquery.png)

    1.7 Logs contain logical SQL generated by the BI Server and the physical SQLs fired against the database.  
    Scroll down the log file and you'll find on top the **Logical SQL** and further down the **Physical SQL** as well.  
    ![Logical SQL](../e-monitoradw/images/managesessionsquerylog.png)  
    > Note: for the physical SQL search the log for "_Sending query to database named_"
    ![Physical SQL](../e-monitoradw/images/managesessionsqueryphi.png)

3. ### Bonus

    Lets test the _**Logical SQL**_ with _**OAC**_ and see the _**Physical SQL**_ in the _**ADW Console**_.

    1.1 **Copy** the **Logical SQL** from the log file. ![Copy Logical SQL](../e-monitoradw/images/copylog.png)

    1.2 Go back to **Console** and Click **Issue SQL** under _Configuration and Administration_ section.  
    ![Issue SQL](../e-monitoradw/images/issuesql.png)

    1.3 Paste the _Logical SQL_ and Click **Issue SQL** button.  
    ![Sessions and Query Cache](../e-monitoradw/images/issuesql2.png)

    1.4 You can quickly see the results (unformatted table below the SQL query).  
    ![Sessions and Query Cache](../e-monitoradw/images/issuesql3.png)

    1.5 Go to the ADW **Service Console > Activity > Monitored SQL**, select your query, right-click and select **Show Details**.  
    ![Show Details](../e-monitoradw/images/adwshowdetails.png)

    1.6 You can see in the **Overview** tab the Physical **SQL Text** which is identical with the one from OAC query log.  
    ![Overview](../e-monitoradw/images/adwshowdetails2.png)

    1.7 You also browse through the other tabs as **Plan Statistics** to see other relevant query information.  
    ![Plan Statistics](../e-monitoradw/images/adwshowdetails3.png)
